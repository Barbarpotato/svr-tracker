...
const groupMember = {
	...
	MAUNG: ["112850698", "133006873", "127229834", "156651628"],
	SuHu: ["131686008", "112850204", "137990204", "156310816"],
	KOPI: ["50725657", "104704718", "131080559", "104704146"],
	"Satgas Lari": ["156440955", "104782290", "131571848", "106400926"],
	"The Munazt": ["104777566", "106128990", "156299639", "131083157"],
	"PS (Pelari Santai)": ["104704203", "104142305", "106084694", "106105959"],
	"Tupai Terbang": [
		"105095022",
		"104676234",
		"168456004", // Musfira
	...
	"The Rising Star": ["106142836", "131082755", "106396953", "156382528"],
	"Kanvas Lari": ["131082566", "106141823", "156646455", "104142273"],
	"Teman Lari Sore": ["156326553", "156379904", "156294430", "106096757"],
	"Lari Sprint": [
		"104977078",
		"156293045",
		"121860615",
	...
	VMA3: ["156363541", "104703436", "112851876", "156632283", "104977227"],
	"Lari Kalo Ingat": [
		"112851267",
		"156753906",
		"156308038",
...
app.get("/tournament-matches", async (req, res) => {
	try {
		...
		for (const phase of tournament_table) {
			...
			for (const date of phaseDates) {
				...
				try {
					...
					if (response.status !== 200) {
						console.log(
							`CSV for ${date} not found (HTTP ${response.status}), skipping`,
						);
						continue;
					}
					...
					await new Promise((resolve, reject) => {
						...
						bufferStream
							.pipe(csvParser())
							.on("data", (row) => {
								// Validasi dan format ulang kolom tanggal
								let rowDate = row.date;
								if (
									rowDate &&
									rowDate.match(/^\d{2}\/\d{2}\/\d{4}$/)
								) {
								...
								if (
									...
									if (
										row.moving_time &&
										typeof row.moving_time === "string"
									) {
										const timeParts =
											row.moving_time.split(":");
										if (timeParts.length === 3) {
											// Format HH:mm:ss
											const [hours, minutes, seconds] =
												timeParts.map(Number);
							...
							.on("error", (err) => {
								console.error(
									`CSV parsing error for ${date}:`,
									err.message,
								);
								reject(
									new Error(
										`Failed to parse CSV for ${date}: ${err.message}`,
									),
								);
							});
					});
					...
					console.log(
						`Error processing CSV for ${date}: ${err.message}, skipping`,
					);
					continue;
				}
			}
			...
			if (!hasValidData) {
				console.log(
					`No valid CSV data for ${phase.title}, skipping phase`,
				);
				continue;
			}
			...
			const matches = (
				phase.title === "Quarter Final"
					? phase.matchlist
					: computeNextPhaseMatches(previousWinners)
			).map(([group1Ids, group2Ids], index) => {
				const group1Name = Object.keys(groupMember).find(
					(key) => groupMember[key] === group1Ids,
				);
				const group2Name = Object.keys(groupMember).find(
					(key) => groupMember[key] === group2Ids,
				);
				...
				const members = {
					[group1Name]: group1Ids.map((id) => {
						const athlete = data.find((a) =>
							a.strava_url.endsWith(id),
						);
						return {
				...
				// Hitung rata-rata jarak dan moving_time untuk setiap tanggal di fase
				const distances = {};
				const movingTimes = {};
				for (const date of phaseDates) {
					...
					let group1TotalMovingTime = 0;
					group1Ids.forEach((id) => {
						const activity = activities.find((row) => {
							const athleteInfo =
								athleteIdMap[row.name.toLowerCase()];
							return athleteInfo && athleteInfo.id === id;
						});
						const member = members[group1Name].find(
							(m) => m.id === id,
						);
						if (activity && member) {
					...
					group2Ids.forEach((id) => {
						...
						const member = members[group2Name].find(
							(m) => m.id === id,
						);
						if (activity && member) {
							member.distance += activity.distance || 0;
					...
					distances[date] = {
						[group1Name]: group1AverageDistance,
						[group2Name]: group2AverageDistance,
					};
					movingTimes[date] = {
						[group1Name]: formatSecondsToHMS(
							group1AverageMovingTime,
						),
						[group2Name]: formatSecondsToHMS(
							group2AverageMovingTime,
						),
					};
				}

				// Hitung total jarak dan moving_time akhir
				const group1TotalDistance = phaseDates
					.reduce((sum, date) => {
						return (
							sum + parseFloat(distances[date]?.[group1Name] || 0)
						);
					}, 0)
					.toFixed(2);
				const group2TotalDistance = phaseDates
					.reduce((sum, date) => {
						return (
							sum + parseFloat(distances[date]?.[group2Name] || 0)
						);
					}, 0)
					.toFixed(2);

				const group1TotalMovingTime = phaseDates.reduce((sum, date) => {
					const time = movingTimes[date]?.[group1Name] || "00:00:00";
					return sum + parseHMSToSeconds(time);
				}, 0);
				const group2TotalMovingTime = phaseDates.reduce((sum, date) => {
					const time = movingTimes[date]?.[group2Name] || "00:00:00";
					return sum + parseHMSToSeconds(time);
				}, 0);
				...
				members[group1Name].forEach((member) => {
					...
					delete member.moving_time_seconds;
				});
				members[group2Name].forEach((member) => {
					member.moving_time = formatSecondsToHMS(
						member.moving_time_seconds,
					);
					delete member.moving_time_seconds;
				...
				return {
					...
					movingTimes,
					totalDistance: {
						[group1Name]: group1TotalDistance,
						[group2Name]: group2TotalDistance,
					},
					totalMovingTime: {
						[group1Name]: formatSecondsToHMS(group1TotalMovingTime),
						[group2Name]: formatSecondsToHMS(group2TotalMovingTime),
					},
					members,
					winner,
				};
			...
			previousWinners = matches
				...
				.map((match) => {
					...
					return match.winner === group1Name
						? groupMember[group1Name]
						: groupMember[match.winner];
				});
		...
		if (!phaseResults.length) {
			return res.status(200).json({
				message: "No tournament phases with valid CSV data found",
			});
		}
		...
		res.status(500).json({
			error: `Error processing tournament matches: ${err.message}`,
		});
	}
});
...
// Helper untuk mengonversi HH:mm:ss ke detik
...
// Helper untuk mengonversi detik ke format HH:mm:ss
	...
	try {
		...
		const athleteResponses = await Promise.all(
			data.map((athlete) =>
				fetch(athlete.strava_url)
					.then((res) => res.text())
					.then((pageData) => {
						const match = pageData.match(
							/<script id="__NEXT_DATA__" type="application\/json">(.*?)<\/script>/s,
						);
						if (!match)
							throw new Error(
								`No script tag found for ${athlete.name}`,
							);
						return {
							athlete,
		...
		const activities = athleteResponses.flatMap(({ athlete, jsonData }) => {
			...
			return (athleteData.recentActivities || [])
				.filter(
					(activity) =>
						activity.startDateLocal === "Today" ||
						activity.startDateLocal === tomorrow ||
						activity.startDateLocal === "Yesterday",
				)
				.map((activity) => ({
					id: athleteid,
			...
			activities.map((activity) =>
				fetch(activity.link)
					...
					.then((pageData) => {
						...
						if (!match)
							throw new Error(
								`No script tag found for activity ${activity.link}`,
							);
						...
						return {
							...
							date: detail_date,
							name: activity.name,
							distance: new Intl.NumberFormat("de-DE").format(
								(activityData.scalars.distance / 1000).toFixed(
									2,
								),
							),
							elevation: activity.elevation,
		...
		const real_activity = activityResponses.filter(
			(result) => result !== null,
		);
		...
		try {
			...
			throw new Error(
				`Unable to fetch or parse CSV from ${csvUrl}: ${err.message}`,
			);
		}
		...
		const uniqueRealActivities = Array.from(
			realActivityMap.values(),
		).filter(
			(activity) =>
		...
		res.setHeader(
			"Content-Disposition",
			`attachment; filename="SVR_${today}.csv"`,
		);
			...
			.on("end", () => {
				...
				const groupStats = Object.keys(groupMember).map((group) => {
					...
					const members = memberIds.map((id) => {
						...
						const distance =
							activity && !isNaN(activity.distance)
					...
					const averageDistance =
						memberCount > 0
							? (totalDistance / memberCount).toFixed(2)
							: "0.00";
				...
				groupStats.sort(
					(a, b) => b.averageDistance - a.averageDistance,
				);
